...
char auth[] = "fbf7908eabc449d2b5d02f2685323294";
int forward;
int reverse;
int left;
int right;
...
//Various settings
float pid_p_gain = 200;                                       //Gain setting for the P-controller (15)
float pid_i_gain = 40;                                      //Gain setting for the I-controller (1.5)
float pid_d_gain = 80;                                       //Gain setting for the D-controller (30)
...
void loop(){
  ...
  if(left == 1){                                            //If the first bit of the receive byte is set change the left and right variable to turn the robot to the left
    pid_output_left += turning_speed;                                       //Increase the left motor speed
    pid_output_right -= turning_speed;                                      //Decrease the right motor speed
  }
  if(right == 1){                                            //If the second bit of the receive byte is set change the left and right variable to turn the robot to the right
    pid_output_left -= turning_speed;                                       //Decrease the left motor speed
    pid_output_right += turning_speed;                                      //Increase the right motor speed
  }

  if(forward == 1){                                            //If the third bit of the receive byte is set change the left and right variable to turn the robot to the right
  ...
  if(reverse == 1){                                            //If the forth bit of the receive byte is set change the left and right variable to turn the robot to the right
  ...
  if((forward == 0) & (reverse == 0)){                                         //Slowly reduce the setpoint to zero if no foreward or backward command is given
